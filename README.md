...
**当前正在执行（2026-09-10，v3方案恢复开发）**：用户已要求执行完整改进方案。原广范围2000轮、支撑336记录轮与低噪声274记录轮及后测均已结束；两次支撑配方都未保住任务精度，不能继续写成等待原2000轮训练。当前先修评估完整时长和正常停止保存，核查真实奖励分项与导入轨迹采样，再按证据选择受控修复与连续学习。保持世界固定末端位姿、单18关节Actor、原验收及临时硬件边界；KMA同步修正文档残留，运行指导已具备相关规则。当前没有通过全身任务验收的策略。见[运行手册](docs/runbook.md)和[验证记录](docs/validation.md)。
