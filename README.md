...
[adaptive sampling对照](artifacts/runs/diagnostic_pose_learning/adaptive_sampling_comparison/README.md) 的单次250轮训练已实际启动，四项固定后测尚未执行。唯一配置差异为`adaptive_sampling=false→true`，复用已完成的uniform softsign250及四项后测作控制，初始化来源与预算相同；采样路径消耗随机数不同，不能称为逐轨迹配对或实际初始动作相同。现有采样EMA仅含位置误差与跌倒，不含朝向／碰撞，EMA改善不能替代位姿任务收益。默认配置、原验收与`trained=false`保持不变，尚无本候选学习结果或采用结论。纯位姿学习稳定前暂缓视觉扩展；其余证据见 [运行手册](docs/runbook.md)，旧246维位置结果保留历史身份。
