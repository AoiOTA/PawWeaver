...
**当前正在执行（2026-09-10）**：[广范围6D末端任务训练](artifacts/runs/diagnostic_pose_learning/wbc_random_training/README.md) 已完成64例独立采样PhysX前测，并启动固定2000轮工程训练。输入为256条不同高度、朝向与距离的末端轨迹，包含必须改变原足支撑位置的目标；前测64例均完整60秒，但位置／朝向RMSE均值为.749939m／2.038435rad，尚无任务能力。训练后进行双引擎跟踪、身体协同和静态到达测试；当前不宣称已学会WBC或硬件有效。此前UMI-inspired 1000轮已完成，局部精度改善，但移动与远距仍跌倒。
