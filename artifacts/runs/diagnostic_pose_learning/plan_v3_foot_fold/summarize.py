...
def build():
    ...
    for side in ("control","candidate"):
        ...
        for engine in ("physx","mujoco"):
            for suite,suite_path in suites.items():
    ...
    for side in bundles:
        for engine in ("physx","mujoco"):
            for suite in ("workspace4","train10"):
                name=f"{side}_{engine}_{suite}"
    ...
    return {"training":training,"comparison_identity":identity,"comparisons":comparisons,"support_geometry":support,
            ...
            "evaluations":{name:{"report_path":str(ROOT/name/"report.json"),"policy_sha256":report["policy_sha256"],
            ...
            "scope":"Only run after operator confirmation of all training/evaluation exits. All requested cases and failures retained; post2 and hold deltas use matched ticks. No task-error group mean replaces failed-case reporting. Focus labels do not filter the other tasks.",
