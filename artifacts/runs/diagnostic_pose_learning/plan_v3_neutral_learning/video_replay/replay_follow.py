...
def render(states,output):
    ...
    camera.distance=2.4
    evidence["camera"]=dict(mode="follow_saved_base_translation",distance_m=2.4,world_azimuth_deg=135,elevation_deg=-22)
    ...
    def frame_at(index):
        ...
        camera.lookat[:]=trace["base"][index]+np.array([0.,0.,.18])
        ...
        lines=[f"PawWeaver B | recorded {metrics['engine']} | {evidence['case_id']} | saved-state MuJoCo geometry replay",
            ...
            'FOLLOW CAMERA | Saved-state replay; no new physics / policy execution / visual control. All frames included.']
