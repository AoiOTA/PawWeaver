"""Local fixed-final-width reward capture; evaluator control path preserved."""
import argparse
import json
...
def main():
    ...
    try:
        ...
        score_state=json.loads((score_root/'score_state.json').read_text())
        config=dict(json.loads((score_root/'config.json').read_text()),domain_randomization=False,adaptive_sampling=False,
        ...
        for first in range(0,len(trajectories),args.num_envs):
            ...
            env.reset(ids)
            env.umi_pose_reward.load_state_dict(score_state)
            ...
            with torch.no_grad():
                for k in range(max(batch_lengths)):
                    ...
                    _,actual_reward,done,extras=env.step(action,auto_reset=False)
                    actual_reward=actual_reward[:count].clone()
                    diagnostics=extras['reward_diagnostics']
                    captured={f'reward_term_{name}':value[:count].clone() for name,value in diagnostics['weighted_nonterminal_terms'].items()}
                    captured.update(actual_reward=actual_reward,
                        nonterminal_preclip=diagnostics['nonterminal_preclip'][:count].clone(),
                        nonterminal_postclip=diagnostics['nonterminal_postclip'][:count].clone())
                    actual_fallen=(extras['fall_height'][:count]|extras['fall_tilt'][:count]).clone()
                    assert torch.allclose(sum(v for key,v in captured.items() if key.startswith('reward_term_')),
                        captured['nonterminal_preclip'],rtol=1e-5,atol=1e-5)
                    assert torch.equal(captured['nonterminal_preclip'],captured['nonterminal_postclip'])
                    assert torch.allclose(actual_reward,.02*captured['nonterminal_postclip']-5*actual_fallen,rtol=1e-5,atol=1e-6)
                    for width in ('position_sigma_m2','orientation_sigma_rad'):
                        assert getattr(env.umi_pose_reward,width)==score_state[width]
                        captured[width]=torch.full_like(actual_reward,score_state[width])
                    captured['terminal_reward']=-5*actual_fallen.to(actual_reward.dtype)
                    state=env.state()
                    ...
                    values.update({key:value.cpu().numpy() for key,value in captured.items()})
                    if k==0:
                        for row in rows:
                            row.update({key:[] for key in captured})
                    step_fallen=actual_fallen.cpu().numpy()
        ...
        save_command_run(args.output,args.suite,manifest,bundle,args.seed,'PhysX',results,
            evaluation=dict(num_envs=args.num_envs,batch_layout=layout,wall_elapsed_seconds=time.perf_counter()-started,
                ...
                command_source='preset_trajectory',score_state=score_state,score_config=str(score_root/'config.json'),
                reward_capture='actual env.step reward and reward_diagnostics weighted terms; fixed final widths'))
        env.close()
    finally:
        import sys
