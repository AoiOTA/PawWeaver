...
**当前运行状态**：[UMI-inspired 训练方案](../artifacts/runs/diagnostic_pose_learning/umi_recipe/README.md) 的1000轮和双引擎test8／far60四项后测已全部实际退出0，仿真资源已释放。训练98,304,000 transitions／20,000次更新、3654.53秒，全部数值有限，累计13,865次跌倒。两引擎local4均完整20秒，位置／朝向RMSE均值分别为PhysX .006117m／.010725rad、MuJoCo .003630m／.010436rad；moving均3/4中途跌倒，far分别6.68秒／1.64秒跌倒。局部精度改善，移动与远距稳定性不支持任务成功，不采用为完整WBC成果。主比较文件已按逐例共同时间窗计算；MuJoCo far没有2秒后的共同窗。下文保留历史实验；本轮为整套方案试验，不作单变量归因。

**当前推进方向**：[全身工作空间目标](../artifacts/runs/diagnostic_pose_learning/wbc_workspace/README.md) 已生成低／高／侧向旋转／远距的世界EE-only测试轨迹，几何见证包含固定站姿臂不可达而全身可达的完整位姿。base=.18m的四足支撑静态姿态与默认.2m高度判据冲突已在实际模型上查明；这是几何证据，尚非动态能力。`7945f9a` 已接通仅采样训练文件库的 `demonstrations_only`、可配置最低基座高度和双引擎同帧跌倒原因记录，CPU检查通过，实际仿真待运行。已按模型几何生成256条训练与64条独立采样测试的广范围6D末端轨迹，包含原地全身仍够不到、需要移动支撑位置的目标。64例fresh PhysX前测实际退出0，均60秒无跌倒，但位置／朝向RMSE均值为.749939m／2.038435rad。沿用现有60秒训练窗口；具体轨迹构造和时序是待训练检验的工程选择。用户举例的采样／插值方法、空间盒、段长、回合长度和人手速度均不作为指定方案或参数依据。[新2000轮训练](../artifacts/runs/diagnostic_pose_learning/wbc_random_training/README.md) 已于2026-09-10 02:34:52 UTC启动，唯一operator顺序执行训练及双引擎后测；另补静态到达16例来检验现有动态测试的能力盲区。UMI实测数据不作为此轮依赖。原plan是参考，具体技术路径依据实际证据调整，见[当前目标与推进依据](plan.md)。
