...
**当前唯一运行**：[UMI-inspired 训练方案](../artifacts/runs/diagnostic_pose_learning/umi_recipe/README.md) 已于2026-09-10 00:49:22 UTC启动固定1000轮训练，执行身份见该目录的运行记录；不要重复启动。计划98,304,000 transitions／20,000次更新，之后顺序执行双引擎 test8／far60 四项后测。250轮仅观察，不中断追加审批。首轮实际优化及课程指标已写出，尚无该方案任务后测结果。下文保留历史实验；本轮是有依据的整套方案试验，不作单变量归因。
