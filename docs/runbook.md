...
本轮实际记录见 [v3奖励信号与受控修复](../artifacts/runs/diagnostic_pose_learning/plan_v3_reward_signal/README.md)。P0实现已本地提交`a0ef8a8`：完整评估步数、正常停止保存、真实奖励分项及可选固定组采样；CPU回归与独立源码审查通过。冻结low-noise251的有效PhysX补采已退出0，128环境×60秒、384,000样本、零学习更新；90.54%裁零，有正向足载荷时仍89.95%裁零。仅将无载荷成本100改1的同状态反算改变约0.055%样本的最终即时奖励，故首轮选择取消裁零的单机制对照，保留其他配置。控制/候选各100轮及开发后测正在执行，尚无学习改善结论。首次补采汇总失败及重试证据均保留。KMA只修正README的委派阻塞旧文，独立本地提交`40cfc16`，未新增规则或重装。
