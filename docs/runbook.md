...
本轮实际记录见 [v3奖励信号与受控修复](../artifacts/runs/diagnostic_pose_learning/plan_v3_reward_signal/README.md)。P0实现已本地提交`a0ef8a8`：完整评估步数、正常停止保存、真实奖励分项及可选固定组采样；CPU回归与独立源码审查通过。冻结low-noise251的有效PhysX补采已退出0，128环境×60秒、384,000样本、零学习更新；90.54%裁零，有正向足载荷时仍89.95%裁零。仅将无载荷成本100改1的同状态反算改变约0.055%样本的最终即时奖励，故首轮选择取消裁零的单机制对照，保留其他配置。控制/候选各100轮及开发后测已完成，结果见上方更新。首次补采汇总失败及重试证据均保留。KMA只修正README的委派阻塞旧文，独立本地提交`40cfc16`，未新增规则或重装。
...
**上一阶段运行状态与停止边界（历史）**：[低噪声最后一轮](../artifacts/runs/diagnostic_pose_learning/wbc_low_noise_learning/README.md)已于2026-09-10 06:43:46 UTC因持续退化提前停止：实际274记录轮、26,935,296 transitions／5480次更新、1071.85秒，全部有限，2185跌倒／8840重置。最新251轮检查点不含后23轮更新；SIGINT后的OS退出0不是1000轮完成，也不证明剩余预算必败。该检查点的原定六项双引擎后测于07:05:07 UTC全部实际退出0，四段PhysX保存状态视频已完成，GPU／仿真资源已释放。PhysX／MuJoCo独立64例均0例位置通过、16／27例跌倒；workspace4均完整且无跌倒，但均0例位置通过、共同窗位置／朝向四例全退步；静态16例均0例到达、12／9例跌倒。本轮far最大水平位移达.962／1.167m、24–36秒保持段无非足净力>5N记录，但high的FL足仍高悬.65–.70m、少足承重与支撑间歇未解决；这与支撑301的far大腿接触问题分别记录。**不采用新策略**，本阶段未达成全身任务验收。当前只整理、提交并由root统一推送PawWeaver远程`main`，随后停止推进；不增加实验、预算、短筛选、速度分支实现或视觉测试。速度＋EE比较本阶段仅完成文献核实。原始中断、训练统计和后测见同目录`interruption.json`、`training_summary.json`、`evaluation_summary.json`、`execution_status.json`。

**已完成2000轮基线**：[广范围6D训练](../artifacts/runs/diagnostic_pose_learning/wbc_random_training/README.md)及双引擎十项后测均实际退出0，最后一项于2026-09-10 05:06:32 UTC结束。训练196,608,000 transitions／40,000次更新、7538.87秒，全部有限，累计22,938跌倒／76,267重置；PhysX／MuJoCo独立64例位置通过11／12例、跌倒24／36例，静态16例位置到达3／2例、跌倒13／14例。原策略也有长期抬腿和跌倒，回到该起点是保留已有跟踪能力并与支撑配方使用同一起点，不代表接受其支撑行为。朝向正式验收阈值未指定，`trained=false`。
...
**支撑配方已完成的失败证据**：[支撑继续学习](../artifacts/runs/diagnostic_pose_learning/wbc_support_learning/README.md)在336记录轮后中断，33,030,144 transitions／6720次更新全部有限，5618跌倒／11942重置；末50轮非终止奖励99.872%裁零，平均位置／朝向误差.25451m／1.44972rad。SIGINT后的OS退出0不等于完成1000轮，也不证明剩余预算必败。最新301轮检查点不含后35轮更新；其六项双引擎后测于06:18:30 UTC全部退出0，四段连续状态回放及视频解码亦退出0。独立64例位置通过两引擎均0，跌倒降为4／7例，但共同窗位置RMSE .088912→.268596m／.087910→.274374m、朝向.735211→1.383604rad／.817120→1.410807rad，明显退化；静态到达仅0／1例。workspace4虽均完整且无跌倒，位置和朝向均全退步；far有持续大腿净接触力，low长期悬足，不能称合理支撑。净接触力未区分地面与自碰撞。固定状态噪声分析仅支持条件PD目标变化机制，不证明随机训练失败的因果。低噪声末轮沿用原256条训练数据、奖励、PD、Actor和.15m／base-up-Z .35终止设置，不重新锚定世界目标、不放宽验收。详见[当前计划](plan.md)与[验证记录](validation.md)。
