...
六轴位置候选已取消且未启动。当前末端位姿接口和初始批量前测见本文开头；完整位姿学习尚无结果，新增朝向验收阈值待确定。单个 18 关节 Actor 保持不变，不启动独立 AS2／Piper-H 预训练；随机教师 CPU 原型仅备用，不具有预训练或技能迁移证据。原日志、指标与验收数值保留，不升级为新任务或硬件有效性证明。

## 尚未完成的验收

- Piper-H 质量/限位/驱动版本，转接板/支撑/线缆，真实相机惯性及标定等 M0 参数。
- 精确硬件执行器阶跃、沉降、足端摩擦和滑移测试。
- 末端位姿策略的有效学习与性能验收；接口和工程前测已具备，新增朝向验收阈值尚未确定。
