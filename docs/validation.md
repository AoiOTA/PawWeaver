...
**当前新增证据**：[UMI-inspired 训练方案](../artifacts/runs/diagnostic_pose_learning/umi_recipe/README.md) 的新初始化与奖励／恢复路径已通过针对性CPU检查，固定1000轮训练已实际启动；首轮完成20次优化、状态有限，并写出真实课程与奖励裁剪指标。完整训练及四项双引擎后测尚未完成，不能据此称任务改善。原标准如下，保持不变。
...
**全身协同的场景含义（用户明确，2026-09-10）**：除需要迈步的远目标，还须覆盖低位、高位、侧向与朝向变化，按任务需要展示腿部支撑、身体升降／倾斜和臂运动共同扩大可操作空间。单Actor输出18关节只是架构条件；混合训练平均误差、水平跟踪或单纯身体下沉不证明这一能力。当前静态目标dz±.08m、大多数合成轨迹固定reset朝向及小高度／朝向变化的训练引用，尚未覆盖该要求；有Z变化的smooth轨迹不能被说成完全平面，也不能替代完整场景验证。基座z<.2m的现行终止条件需按实际模型净空检查能否区分正常低姿态和倒地，不能直接放宽来制造通过。这里澄清原能力目标，不新增数值通过阈值、不改变上表原标准。
